...
class AriaDriver extends EventEmitter {
   ...
   */
  async use(locator) {
    await this._use(locator, await this._findOne(locator));
  }

  async _use(locator, target) {
   ...
   * element}.
   *
   * @param {string} locator - a CSS selector describing the element to {@link
  ...
  async openPopup(locator) {
    ...
    await this._use(locator, target);
   ...
   *         AriaDriver#patience the "patience" duration}
   */
  async pushButton(locator) {
    const target = await this._findOne(locator);
    /* istanbul ignore next */
    const {
      ariaPressed, ariaLabel, ariaLabelledBy, role, tagName, innerText
      ...
      return {
        ...
        role: arguments[0].getAttribute('role'),
    ...
    if (!/^button$/i.test(tagName) && !/^button$/i.test(role)) {
      throw new AriaDriverError(
        'ARIADRIVER-INVALID-MARKUP',
        null,
